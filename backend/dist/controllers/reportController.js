...
exports.getCashFlowReport = exports.getFinancialReport = exports.getSalesReport = exports.getDashboardStatsInternal = exports.getReports = void 0;
...
const getReports = async (req, res) => {
    try {
        ...
        if (endpoint === 'dashboard') {
            return await (0, exports.getDashboardStatsInternal)(req, res);
        }
...
const getDashboardStatsInternal = async (req, res) => {
    try {
        ...
        const [totalCustomers, activeCustomers, totalQuotes, pendingQuotes, approvedQuotes, totalInvoices, paidInvoices, unpaidInvoices, overdueInvoices, monthlyRevenue, yearlyRevenue, totalRevenue] = await Promise.all([
            // Clients
...
const getSalesReport = async (req, res) => {
    try {
        ...
        // Ventes par période
        const salesByPeriod = await getSalesByPeriod(invoiceFilter, groupBy);
        ...
        const productsData = await Promise.all(salesByProduct.map(async (item) => {
        ...
        const customersData = await Promise.all(salesByCustomer.map(async (item) => {
...
async function getPipelineData(quoteFilter) {
    const stages = [
...
async function getSalesByUser(invoiceFilter) {
    ...
    const usersData = await Promise.all(salesByUser.map(async (sale) => {
...
async function getRecentActivities(serviceId) {
    const activities = [];
    // Derniers devis
...
async function getMonthlyRevenue(invoiceFilter) {
    const currentDate = new Date();
    const months = [];
    for (let i = 11; i >= 0; i--) {
...
async function getTopCustomers(customerFilter, invoiceFilter) {
...
async function getAgingReport(invoiceFilter) {
    const currentDate = new Date();
...
async function getSalesByPeriod(invoiceFilter, groupBy) {
...
async function getMonthlyFinancialData(start, end) {
