...
exports.validateProspect = exports.getProspectionStats = exports.moveProspectStage = exports.deleteProspect = exports.updateProspect = exports.createProspect = exports.getProspectById = exports.getProspects = void 0;
...
const createProspect = async (req, res) => {
    try {
        ...
        const { companyName, contactName, position, email, phone, address, website, industry, companySize, estimatedValue, priority, source, notes, hasBudget, isDecisionMaker, hasNeed, timeline } = req.body;
...
const moveProspectStage = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.moveProspectStage = moveProspectStage;
const getProspectionStats = async (req, res) => {
    try {
        let whereClause = {};
        // Filtrage par service pour les non-admin
        if (!['ADMIN', 'GENERAL_DIRECTOR'].includes(req.user.role)) {
        ...
        const [totalProspects, activeProspects, convertedProspects, byStage, byPriority] = await Promise.all([
            prisma.prospect.count({ where: whereClause }),
...
exports.getProspectionStats = getProspectionStats;
// Fonction utilitaire pour convertir un prospect en client
async function convertProspectToCustomer(prospect, userId) {
    try {
...
exports.validateProspect = [
    (0, express_validator_1.body)('companyName').notEmpty().withMessage('Nom de l\'entreprise requis'),
    (0, express_validator_1.body)('contactName').notEmpty().withMessage('Nom du contact requis'),
    (0, express_validator_1.body)('email').optional().isEmail().withMessage('Email invalide'),
    (0, express_validator_1.body)('priority').optional().isIn(['A', 'B', 'C']).withMessage('Priorité invalide'),
    (0, express_validator_1.body)('estimatedValue').optional().isFloat({ min: 0 }).withMessage('Valeur estimée invalide')
];
