...
export const LeaveManagement: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => leaveService.getAll({ page, limit: 10, search, status: statusFilter })
  });
  ...
  const handleApprove = async (request: any) => {
    ...
    try {
      await approveMutation.mutateAsync({ id: request.id, comments: comments || undefined });
    } catch (error) {
  ...
  const handleReject = async (request: any) => {
    ...
    try {
      await rejectMutation.mutateAsync({ id: request.id, comments });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Gérez les demandes de congé et les approbations</p>
        </div>
              ...
              return (
                <tr key={request.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                    <div>
                      <div>Du {new Date(request.startDate).toLocaleDateString()}</div>
                      <div>Au {new Date(request.endDate).toLocaleDateString()}</div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
