...
import { z } from 'zod';
import { X, Plus, Trash2, Receipt, Calendar, User, FileText } from 'lucide-react';
...
export const CreateInvoiceModal: React.FC<CreateInvoiceModalProps> = ({ 
  ...
  const { data: quotes } = useQuery({
    ...
    queryFn: () => quoteService.getAll({ status: 'APPROVED_BY_DG', limit: 100 })
  });
  ...
  const {
    ...
    defaultValues: {
      ...
      paymentTerms: 30,
      quoteId: fromQuoteId,
      items: [{ description: '', quantity: 1, unitPriceHt: 0, discountRate: 0, vatRate: 18 }]
    }
  });
  ...
  const createInvoiceMutation = useMutation({
    mutationFn: (data: CreateInvoiceFormData) => {
      if (fromQuoteId) {
        return invoiceService.create({ endpoint: `from-quote/${fromQuoteId}` });
      }
  ...
  const loadQuoteData = async (quoteId: number) => {
    try {
      ...
      setValue('customerAddressId', quote.customerAddressId);
      setValue('terms', quote.terms);
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-screen overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900 flex items-center">
            ...
            {fromQuoteId ? 'Créer une Facture depuis un Devis' : 'Créer une Facture'}
          </h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {!fromQuoteId && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Devis de référence (optionnel)</label>
                <div className="relative">
                  <FileText className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                  <select
                    {...register('quoteId', { valueAsNumber: true })}
